    ...
    public Object encode(IResponseMessage aResponseMessage) throws ServerException {
        try {
            ...
            if (matchReturn.mCode == ResponseCode.FAILURE) {
                ...
                encodingObj.put("totalTime", matchReturn.totalTime);
                encodingObj.put("begin_id", matchReturn.begin_id);
                if (matchReturn.zoneID == ZoneID.BACAY) {
                    JSONArray arrValues = new JSONArray();
                    //mLog.info("[DEBUG] " + matchReturn.players.size());
                    for (SimplePlayer player : matchReturn.players) {
                ...
                if (matchReturn.zoneID == ZoneID.PHOM) {
                    JSONArray arrValues = new JSONArray();
